...
<x-dynamic-component :component="$getEntryWrapperView()" :entry="$entry">
    <div class="file-viewer p-4 space-y-4">
        @foreach ($getState() as $media)
            <div
                class="file-entry flex items-center p-4 border rounded-lg shadow-md bg-white hover:bg-gray-100 transition-colors">
                ...
                <div class="file-info flex flex-col items-start">
                    <strong class="text-lg font-semibold mb-2">{{ $media->name }}</strong>
                    <a href="{{ $media->getUrl() }}" target="_blank" class="text-blue-500 hover:underline">Ver archivo</a>
                </div>
            </div>
        @endforeach
    </div>
